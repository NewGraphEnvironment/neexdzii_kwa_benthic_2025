# map_ems-stations.R
#
# Produces fig/map_ems-stations.png — EMS water quality monitoring stations
# on the Neexdzii Kwa. Same cartographic style as map_benthic-sites.R but
# swaps in EMS stations instead of benthic sampling sites.
#
# Run data_map-study-area.R first (requires DB) to populate data/spatial/ cache.
# Run ems_prep.R first to generate data/processed/ems_stations.csv.
#
# Usage: Rscript scripts/map_ems-stations.R
...
# Neexdzii Kwa sub-watershed only
...
# Filter to stream/river stations (type 21) with coordinates
ems_csv <- ems_csv |>
  filter(!is.na(lat) & !is.na(lon),
         location_type == "21") |>
  mutate(short_name = stringr::str_replace(monitoring_location, "PE\\d+\\s*", "") |>
           stringr::str_to_title() |>
           stringr::str_trunc(30))

ems_sites <- st_as_sf(ems_csv, coords = c("lon", "lat"), crs = 4326)
...
bbox_clip <- st_bbox(neexdzii)
bbox_clip["ymax"] <- bbox_clip["ymax"] + 0.08
bbox_clip["ymin"] <- bbox_clip["ymin"] - 0.08
bbox_clip["xmin"] <- bbox_clip["xmin"] - 0.06
bbox_clip["xmax"] <- bbox_clip["xmax"] + 0.06
bbox_clip_sf <- st_as_sfc(bbox_clip) |> st_set_crs(4326)
...
m <- tm_shape(basemap_stars, bbox = bbox) +
  ...
  # Neexdzii Kwa watershed boundary
  tm_shape(neexdzii) +
  ...
  tm_text("short_name", size = 0.55, col = "grey20",
          fontface = "bold",
          options = opt_tm_text(
            point.label = TRUE,
  ...
  tm_text("name", size = 0.55, col = "#8b0000",
          fontface = "italic",
          options = opt_tm_text(
            point.label = TRUE,
            point.label.method = "SANN",
  ...
  tm_add_legend(
    type = "symbols",
    labels = c("EMS monitoring station", "Potential point source"),
    fill   = c("#2ca02c", "#e74c3c"),
    shape  = c(21, 23),
    size   = c(0.8, 0.8),
    col    = c("grey20", "grey20")
  ) +
...
tmap_save(m, "fig/map_ems-stations.png", width = 8.5, height = 11, dpi = 200)
system("sips -s dpiWidth 200.0 -s dpiHeight 200.0 fig/map_ems-stations.png")
...
main_img   <- image_read("fig/map_ems-stations.png")
...
main_img |>
  ...
  image_write("fig/map_ems-stations.png")

system("sips -s dpiWidth 200.0 -s dpiHeight 200.0 fig/map_ems-stations.png")
cat("Saved to fig/map_ems-stations.png\n")
