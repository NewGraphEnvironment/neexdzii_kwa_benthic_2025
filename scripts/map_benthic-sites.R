# map_benthic-sites.R
#
# Produces fig/map_benthic-sites.png — sampling locations on the Neexdzii Kwa.
# Zooms to Neexdzii Kwa sub-watershed only (unlike map_study-area.R which shows
...
# Clip features to bbox extent (simpler than polygon intersection, avoids topology errors)
...
# Stream label points (5th order+ named, within extent)
...
# Towns within Neexdzii Kwa extent
towns <- st_sf(
...
# --- Bounding box ---------------------------------------------------------
# Zoom to Neexdzii Kwa with modest padding
...
m <- tm_shape(basemap_stars, bbox = bbox) +
  ...
  tm_text("site", size = 0.70, col = "grey20",
          fontface = "bold",
          options = opt_tm_text(
            point.label = TRUE,
...
tmap_save(m, "fig/map_benthic-sites.png", width = 8.5, height = 11, dpi = 200)
system("sips -s dpiWidth 200.0 -s dpiHeight 200.0 fig/map_benthic-sites.png")
...
main_img   <- image_read("fig/map_benthic-sites.png")
...
main_img |>
  ...
  image_write("fig/map_benthic-sites.png")

system("sips -s dpiWidth 200.0 -s dpiHeight 200.0 fig/map_benthic-sites.png")
cat("Saved to fig/map_benthic-sites.png\n")
