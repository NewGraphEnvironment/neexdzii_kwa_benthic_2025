...
3. **Expand the monitoring network.** The current three-site design covers the mainstem nutrient gradient between Houston and McQuarrie Creek but leaves gaps in our understanding of the broader watershed. The Equity Silver Mine straddles the drainage divide between Foxy Creek (flowing north into Maxan Creek) and Bessemer Creek (flowing south into Buck Creek) [@nijman1996Waterquality], and both drainage paths warrant monitoring. Proposed expansion sites are summarized in Table \@ref(tab:tab-proposed-sites-cap) and shown in Figure \@ref(fig:map-proposed-sites).

<br>

```{r tab-proposed-sites-cap, results="asis"}
my_caption <<- "Proposed monitoring sites for expanded benthic assessment of the Neexdzii Kwa watershed. Sites are designed to capture mine drainage, forestry and agricultural influences, point-source impacts, and chinook-bearing reaches."
my_tab_caption(tip_flag = FALSE)
```

```{r tab-proposed-sites}
tibble::tribble(
  ~Stream, ~Reach, ~Rationale,
  "Neexdzii Kwa mainstem", "Above McQuarrie confluence", "Test whether reference-quality conditions persist beyond cold-water tributary influence",
  "Buck Creek", "Below Bessemer confluence", "Mine drainage — south side of Equity Silver divide; chinook-bearing reach",
  "Buck Creek", "Lower reach near Bulkley confluence", "Chinook spawning and rearing habitat; integrates upstream influences",
  "Foxy Creek", "Upper reach near mine", "Mine drainage — north side of Equity Silver divide",
  "Maxan Creek", "Near Bulkley Lake", "Forestry and agricultural land use influences",
  "Richfield Creek", "Near Highway 16", "Legacy concentrate shed site; documented chinook use; fish passage concerns",
  "Byman Creek", "Lower reach", "Documented degradation from agriculture and transportation"
) |>
  my_dt_table(cols_freeze_left = 0, page_length = 10)
```

<br>

```{r map-proposed-sites, fig.cap='Proposed monitoring sites for expanded benthic assessment of the Neexdzii Kwa watershed. Blue triangles indicate existing 2025 sampling sites; orange stars show proposed expansion sites. Site rationale is summarized in Table \\\\@ref(tab:tab-proposed-sites-cap).', out.width="100%", eval=FALSE}
